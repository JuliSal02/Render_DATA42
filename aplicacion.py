...
#tiutlo
st.header("Relación entre variables iris", divider="gray")
...
v = st.multiselect(
    label= 'selecciona maximo 2 variables',
    options= opciones,
    max_selections = 2 ,
)

# Boton para ejecutar los graficos 
...
if analisis_b:
    try:
        ...
        with col1:
            hist_plot01 = px.histogram(df, x=v[0], title= f'Ditribución {v[0]}',color = "type")
        ...
        with col2:
            hist_plot02 = px.histogram(df, x = v[1], title= f"Distribución de {v[1]}", color="type")
        ...
        disp_plor = px.scatter(df, x = v[0], y=v[1], color = 'type', title=f'Disperción {v[1]} vs {v[0]}')
        st.plotly_chart(disp_plor, use_container_width= True)
